...
def _verify_cas2(ticket, service):
    ...
    try:
        ...
        if tree[0].tag.endswith('authenticationSuccess'):
            id = tree[0][1].text
            db = tree[0][2].text
            username = tree[0][3].text
            user_email = tree[0][4].text
            user_lang = tree[0][5].text
            password = tree[0][6].text
            return tree[0][0].text,{"id":id,"db":db,"username":username,"user_email":user_email,"user_lang":user_lang,"password":password}
        else:
            return None, None
    finally:
        page.close()
